...
export default function ClientBookings() {
  ...
  const fetchBookings = async () => {
    try {
      ...
      toast.error('Erro ao carregar as suas marcações.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const cancelBooking = async () => {
    ...
    try {
      await api.patch(`/bookings/${bookingToCancel}/status`, 4, { // 4 = Cancelada
  ...
  if (loading) return <div className="p-12 text-center animate-pulse">Carregando o seu calendário...</div>;
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 relative">
      ...
      <div className="bg-card border border-border rounded-2xl p-6 shadow-sm flex flex-wrap justify-between items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm font-medium">
            Visualize e gira as suas marcações semanais.
          </p>
        </div>
                    ...
                    return (
                      ...
                      >
                        ...
                        <p className="text-[10px] text-muted-foreground flex items-center gap-1">
                          <Store className="h-2.5 w-2.5" /> {booking.negocio?.nome || 'Estabelecimento'}
                        </p>
                      </button>
                    );
                  })
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-card w-full max-w-md rounded-3xl shadow-2xl border border-border overflow-hidden animate-in zoom-in-95 duration-300">
            <div className="relative p-6 bg-primary text-primary-foreground">
               ...
               <h3 className="text-2xl font-display font-bold">Detalhes da Minha Marcação</h3>
               <p className="opacity-80 text-sm">Marcação #{selectedBooking.id}</p>
            </div>
            ...
            <div className="p-8 space-y-6">
               <div className="space-y-4">
                  <div className="flex items-center gap-4 p-4 bg-muted/50 rounded-2xl border border-border">
                     ...
                     <div>
                        ...
                        <p className="text-[10px] text-muted-foreground">Profissional: {selectedBooking.profissional?.nome || '—'}</p>
                     </div>
                  </div>
                  <div className="flex items-center gap-4 p-4 bg-muted/50 rounded-2xl border border-border">
                     <div className="w-12 h-12 bg-primary/10 rounded-xl flex items-center justify-center text-primary"><CalendarIcon className="h-6 w-6" /></div>
                     <div>
                        <p className="text-[10px] uppercase font-bold text-muted-foreground tracking-widest">Data e Hora</p>
                        <p className="font-bold text-foreground">{format(parseISO(selectedBooking.dataHoraInicio), "dd 'de' MMMM, yyyy", { locale: pt })}</p>
                        <p className="text-xs text-muted-foreground flex items-center gap-1"><Clock className="h-3 w-3" /> {format(parseISO(selectedBooking.dataHoraInicio), "HH:mm")}</p>
                     </div>
                  </div>
                  <div className="flex items-center gap-4 p-4 bg-muted/50 rounded-2xl border border-border">
               ...
               <div className="pt-4 border-t border-border text-center">
                  {(() => {
                    ...
                    if (currentStatusId >= 3) { // 3=Concluída, 4=Cancelada, 5=Rejeitada
                      return (
                        <div className={`py-4 px-6 rounded-2xl font-bold flex items-center justify-center gap-2 ${info.color}`}>
                           ...
                           Esta marcação está {info.label}
                        </div>
                      );
                    }
                    return (
                      <div className="space-y-4">
                        <p className="text-sm font-medium text-muted-foreground mb-3">Deseja cancelar esta marcação?</p>
                        <button 
                          onClick={() => setBookingToCancel(selectedBooking.id)} 
                          className="w-full flex items-center justify-center gap-2 py-4 bg-red-600 text-white rounded-2xl font-bold hover:bg-red-700 shadow-lg shadow-red-200 transition-all active:scale-95"
                        >
                          <XCircle className="h-5 w-5" /> Cancelar Marcação
                        </button>
                        <p className="text-[10px] text-muted-foreground italic">O profissional será notificado automaticamente.</p>
                      </div>
                    );
                  })()}
               </div>
      ...
      <ConfirmDialog 
        ...
        message="Tem a certeza que deseja cancelar esta marcação? Esta ação não pode ser desfeita e o horário será libertado."
